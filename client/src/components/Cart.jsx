...
const Cart = () => {
  ...
  const handleCheckOut = () => {
    ...
    axios
      ...
      .then((res) => {
        console.log("handle cart", res);
        if (res.data.url) {
          window.location.href = res.data.url;
        }
      })
      .catch((err) => console.log("eerror in sending", err));
  };
        ...
        {cart ? (
          <>
            ...
            <div className="bg-zinc-800 rounded-t-[60px] w-full h-[35%] flex flex-col items-center justify-center px-4 py-2 mb-2 mt-3 gap-16">
              ...
              <motion.div
                {...buttonClick}
                className="bg-orange-400 pb-30   text-center cursor-pointer w-[70%] px-4 py-4 text-xl text-headingColor font-bold hover:bg-orange-600 drop-shadow-md rounded-2xl"
